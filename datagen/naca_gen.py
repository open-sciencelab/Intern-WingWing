...
def generate_params(n_digits, n_samples):
    airfoils = []

    if n_digits == 4:
        M_values = np.random.uniform(0, 7, n_samples)
        P_values = np.random.uniform(0, 7, n_samples)
        P_values[P_values < 1.5] = 0
        XX_values = np.random.uniform(5, 20, n_samples)
        
        for i in range(n_samples):
            params = [M_values[i], P_values[i], XX_values[i]]
            airfoils.append(params)
            
    elif n_digits == 5:
        L_values = np.random.uniform(0, 4, n_samples)
        P_values = np.random.uniform(3, 8, n_samples)
        Q_values = np.random.choice([0, 1], n_samples)
        XX_values = np.random.uniform(5, 20, n_samples)
        
        for i in range(n_samples):
            params = [L_values[i], P_values[i], Q_values[i], XX_values[i]]
            airfoils.append(params)
...
def process_file(params,idx,label):
    naca = naca_generator(params, nb_samples = 128, scale = 1, origin = (0, 0), cosine_spacing = True, verbose = False, CTE = True)
    with open(f'{root_path}/{idx:04d}_{label}.dat', 'w') as f:
        for x, y in naca:
            f.write(f'{x} {y}\n')
 
if __name__ == '__main__':
    n_samples = 5000
    airfoils_4 = generate_params(4, n_samples)
    airfoils_5 = generate_params(5, n_samples)
    root_path = 'data/airfoil/naca_gen'
    os.makedirs(root_path, exist_ok=True)
    for i in tqdm(range(n_samples)):
        process_file(airfoils_4[i],i,'4')
    print('4 digits done')
    for i in tqdm(range(n_samples)):
        process_file(airfoils_5[i],i,'5')
    print('5 digits done')
